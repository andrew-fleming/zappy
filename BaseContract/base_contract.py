...
class BaseContract:
        ...
        try:
            ...
            if coordinator is not None:
                ...
                checksum_coor_address = \
                    self.w3.toChecksumAddress(self.coor_artifact['networks'][str(self.network_id)]['address'])

            self.coordinator = self.w3.eth.contract(address=checksum_coor_address, abi=self.coor_artifact['abi'])

            if address is not None:
                self.address = address
            else:
                self.address = self.artifact['networks'][str(self.network_id)]['address']
    ...
    async def get_contract(self):
        ...
        self.contract = self.w3.eth.contract(address=contract_address, abi=self.artifact['abi'])
